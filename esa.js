...
const urls = ["https://app.esamarathon.dev/horaro-proxy/v2/esa/schedule/2024-summer"];
...
function prettyLength(length) {
  return [String(Math.floor(length / 3600)), String((length / 60) % 60), String(length % 60).padStart(2, "0")].join(
    ":"
  );
}
...
function streamToGames(stream) {
  ...
  return scheduleJson.data.map((game) => {
    ...
    return {
      ...
      runners: game.players.map(deMarkdown).join(", "),
...
function getEsaSchedules(callback) {
  fetch({ url: urls[0] }, (err, stream1) => {
    if (err) {
      ...
      callback(null, streamToGames(stream1));
    }
  });
}
