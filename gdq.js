...
async function fetchRuns() {
  const url =
    "https://tracker.gamesdonequick.com/tracker/api/v2/events/56/runs/";
...
export async function* getSchedule(
  ...
  for (const run of runs.results) {
    ...
    if (!done) {
      ...
      const isToday = maxDate.isAfter(start); // start.isBefore(moment("2025-07-07 07:00"));

      if (isToday) {
        yield {
          start,
          title,
          runners: runnersArray.map(({ name }) => name).join(", "),
          estimate,
          ends,
          done,
        };
