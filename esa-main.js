...
function eventName(acronym) {
  ...
  return (
    spaceBetween(
      "esa" === acronym ? `ESA ${isWinter ? "WINTER" : "SUMMER"}` : `${isWinter ? "A" : "S"}GDQ`,
      ...
      const runsToday = receiptFormatter(
        runs
          ...
          .reduce((list, run) => {
            ...
            const runTimes = `    ${run.start.format(RUN_FORMAT)} - ${run.ends.format(RUN_FORMAT)}`;
            const spacePadding = Math.max(maxWidth - (estimate.length + runTimes.length), 0);
            const runTitle = BONUS_GAME.test(run.title) ? "??? Bonus\n    ???" : run.title;
            return `${list}\n( ) ${runTitle}\n    ${run.runners}\n${runTimes}${spaces(
              spacePadding
            )}${estimate}\n\n--------------------------------`;
          }, eventName(eventCode))
